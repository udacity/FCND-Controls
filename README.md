...
For this step, you'll be writing the nested low-level controller to achieve the waypoint following needed to complete the backyard flyer project. To do this you'll be filling in methods in the 'controller.py' class. Using the non-linear dynamics and control from the lessons, you'll write the control code for each of the controller parts showing control diagram shown above. The minimum requirements for a passing submission to this step include:

* (body_rate_control) A proportional controller on body rates to commanded moments
* (altitude_control) A linear altitude controller that uses both the down position and the down velocity to command thrust. This controller needs to include the non-linear transformation from local frame to body frame
* (yaw_control) A linear/proportional heading controller to yaw rate commands (non-linear transformation not required)
* (roll_pitch_control) A reduced attitude controller taking in local acceleration commands and outputs body rate commands. This controller needs to take into the nonlinear transformation from local frame to body frame.
...
The trajectory following may require additional retuning because the gains set for the waypoint following may be too high/low to effectively follow the trajectory. The ability to follow a trajectory will be tested with an test trajectory. The trajectory is made up several straight-line segments of varying speed and curved segments. Successful completion of this step include:

* The drone flies the test trajectory faster than the specified threshold
* The maximum horizontal error is less than the specified threshold
* The maximum vertical error is less than the specified threshold
...
## Step 11: Write it up!
When you're finished, complete a detailed writeup of your solution and how you addressed each step.
